...
.PARAMETER LicenseCostPerUser
    Monthly license cost per user in USD (default: 36 for M365 E3)
    Common: M365 E3=$36, M365 E5=$57, Intune standalone=$8
    Note: Prices increase July 2026 to E3=$39, E5=$60
...
.EXAMPLE
    .\Invoke-IntuneDeviceLifecycle.ps1 -CsvPath ".\devices.csv" -LicenseCostPerUser 57 -LicenseName "M365 E5"
.EXAMPLE
...
foreach ($dev in $devices) {
    ...
    $upn = if($m.UPN){$dev.($m.UPN).Trim().ToLower()} else {""}
...
$wastedUPNs = [System.Collections.Generic.HashSet[string]]::new()
...
# Per-phase wasted user counts (only users with zero active devices)
$warnWastedUsers = @($staleDevices | Where-Object { $_.Phase -eq "Warn" -and $_.UPN -and $wastedUPNs.Contains($_.UPN) } | Select-Object -ExpandProperty UPN -Unique).Count
$disableWastedUsers = @($staleDevices | Where-Object { $_.Phase -eq "Disable" -and $_.UPN -and $wastedUPNs.Contains($_.UPN) } | Select-Object -ExpandProperty UPN -Unique).Count
$retireWastedUsers = @($staleDevices | Where-Object { $_.Phase -in @("Retire","Stale entry") -and $_.UPN -and $wastedUPNs.Contains($_.UPN) } | Select-Object -ExpandProperty UPN -Unique).Count
$warnWasteCost = [math]::Round($warnWastedUsers * $annualPerUser, 0)
$disableWasteCost = [math]::Round($disableWastedUsers * $annualPerUser, 0)
$retireWasteCost = [math]::Round($retireWastedUsers * $annualPerUser, 0)
$immediateRecovery = $retireWasteCost
...
$allPhases | Select-Object `
    ...
    @{N='LicenseWasted';E={if($_.UPN -and $wastedUPNs.Contains($_.UPN)){'Yes'}else{'No'}}},
...
<div class="cost-g">
<div class="cost-i"><div class="cost-iv" style="color:var(--ac)">`$$($warnWasteCost.ToString('N0'))</div><div class="cost-il">Warn ($warnWastedUsers users)</div></div>
<div class="cost-i"><div class="cost-iv" style="color:var(--am)">`$$($disableWasteCost.ToString('N0'))</div><div class="cost-il">Disable ($disableWastedUsers users)</div></div>
<div class="cost-i"><div class="cost-iv" style="color:var(--rd)">`$$($retireWasteCost.ToString('N0'))</div><div class="cost-il">Retire/No-sync ($retireWastedUsers users)</div></div>
</div>
...
function xUsers(){const umap={};D.forEach(d=>{if(!d.upn||!d.w)return;if(!umap[d.upn])umap[d.upn]={n:d.u,cnt:0,devs:[]};umap[d.upn].cnt++;umap[d.upn].devs.push(d.dn)});
const h=['UPN','DisplayName','StaleDevices','DeviceList','LicenseWasted','EstMonthlyCost','EstAnnualCost','LicensePlan'];
const r=Object.entries(umap).sort((a,b)=>b[1].cnt-a[1].cnt).map(([k,v])=>[k,v.n,v.cnt,v.devs.join('; '),'Yes - zero active devices','$'+(TWS/Math.max(WU,1)/12).toFixed(2),'$'+(TWS/Math.max(WU,1)).toFixed(2),LC]);
dl(mkcsv(h,r),'WastedLicenses.csv')}
function xPBI(){const h=['Serial','Device','UPN','User','Platform','OS','Model','DaysInactive','LastSync','Enrolled','Compliance','Owner','Phase','Priority','Autopilot','HasSyncDate','LicenseWasted'];
const r=D.map(d=>[d.sr,d.dn,d.upn,d.u,d.p,d.ov,d.m,d.d===-1?'':d.d,d.sy,d.en,d.c,d.ow,d.ph,d.pr,d.ap?'Yes':'No',d.d===-1?'No':'Yes',d.w?'Yes':'No']);dl(mkcsv(h,r),'PowerBI_StaleDevices.csv')}
rt();
</script></body></html>
"@
